...
def get_sts_token(token: str, endpoint: str):
    response = requests.post(
        f'{endpoint}/?Action=AssumeRoleWithWebIdentity&WebIdentityToken={token}&Version=2011-06-15&DurationSeconds={2592000}',
        verify=False,
        timeout=5
    )
    ...
    if response.status_code == 200:
        ...
        credentials = {'access_key': access_key.text,
                       'secret_key': secret_key.text, 'sts_token': session_token.text}
        return credentials
    else:
