...
user1 = {'login': 'user1', 'password': '123', 'token': client.get(
    '/auth', auth=('user1', '123')).json()['token']}
...
def test_system_logs():
    ...
    assert last_action['message'] == {'login': 'user1'}
    assert last_action['date_time'] == datetime.now().isoformat("T", "seconds")
